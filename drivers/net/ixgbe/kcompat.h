...
#define vlan_ethhdr _kc_ixgbe_vlan_ethhdr
struct _kc_ixgbe_vlan_hdr {
	__be16		h_vlan_TCI;
	__be16		h_vlan_encapsulated_proto;
};
#define vlan_hdr _kc_ixgbe_vlan_hdr
...
#define ethtool_regs _kc_ixgbe_ethtool_regs
/* for passing big chunks of data */
struct _kc_ixgbe_ethtool_regs {
	u32 cmd;
...
#define ethtool_coalesce _kc_ixgbe_ethtool_coalesce
struct _kc_ixgbe_ethtool_coalesce {
...
#define ethtool_ringparam _kc_ixgbe_ethtool_ringparam
struct _kc_ixgbe_ethtool_ringparam {
...
#define ethtool_pauseparam _kc_ixgbe_ethtool_pauseparam
struct _kc_ixgbe_ethtool_pauseparam {
...
static inline int _kc_ixgbe_test_and_clear_bit(int nr, volatile unsigned long *addr)
{
...
#define test_and_clear_bit(nr, addr) _kc_ixgbe_test_and_clear_bit(nr, addr)

static inline int _kc_ixgbe_test_and_set_bit(int nr, volatile unsigned long *addr)
{
	unsigned long mask = BIT_MASK(nr);
	unsigned long *p = ((unsigned long *)addr) + BIT_WORD(nr);
	unsigned long old;
...
#define test_and_set_bit(nr, addr) _kc_ixgbe_test_and_set_bit(nr, addr)
...
#define pci_set_dma_mask _kc_ixgbe_pci_set_dma_mask
int _kc_ixgbe_pci_set_dma_mask(struct pci_dev *dev, dma_addr_t mask);
#endif

#ifndef pci_request_regions
#define pci_request_regions _kc_ixgbe_pci_request_regions
int _kc_ixgbe_pci_request_regions(struct pci_dev *pdev, char *res_name);
#endif

#ifndef pci_release_regions
#define pci_release_regions _kc_ixgbe_pci_release_regions
void _kc_ixgbe_pci_release_regions(struct pci_dev *pdev);
#endif
...
#define alloc_etherdev _kc_ixgbe_alloc_etherdev
struct net_device * _kc_ixgbe_alloc_etherdev(int sizeof_priv);
#endif

#ifndef is_valid_ether_addr
#define is_valid_ether_addr _kc_ixgbe_is_valid_ether_addr
int _kc_ixgbe_is_valid_ether_addr(u8 *addr);
#endif
...
#define pci_set_power_state _kc_ixgbe_pci_set_power_state
int _kc_ixgbe_pci_set_power_state(struct pci_dev *dev, int state);
#endif

#ifndef pci_enable_wake
#define pci_enable_wake _kc_ixgbe_pci_enable_wake
int _kc_ixgbe_pci_enable_wake(struct pci_dev *pdev, u32 state, int enable);
#endif

#ifndef pci_disable_device
#define pci_disable_device _kc_ixgbe_pci_disable_device
void _kc_ixgbe_pci_disable_device(struct pci_dev *pdev);
#endif
...
int _kc_ixgbe_snprintf(char * buf, size_t size, const char *fmt, ...);
#define snprintf(buf, size, fmt, args...) _kc_ixgbe_snprintf(buf, size, fmt, ##args)
int _kc_ixgbe_vsnprintf(char *buf, size_t size, const char *fmt, va_list args);
#define vsnprintf(buf, size, fmt, args) _kc_ixgbe_vsnprintf(buf, size, fmt, args)
#else /* 2.4.8 => 2.4.9 */
...
#ifndef pci_map_page
#define pci_map_page _kc_ixgbe_pci_map_page
u64 _kc_ixgbe_pci_map_page(struct pci_dev *dev, struct page *page, unsigned long offset, size_t size, int direction);
#endif

#ifndef pci_unmap_page
#define pci_unmap_page _kc_ixgbe_pci_unmap_page
void _kc_ixgbe_pci_unmap_page(struct pci_dev *dev, u64 dma_addr, size_t size, int direction);
#endif
...
#define netif_poll_disable(x) _kc_ixgbe_netif_poll_disable(x)
static inline void _kc_ixgbe_netif_poll_disable(struct net_device *netdev)
{
...
#define netif_poll_enable(x) _kc_ixgbe_netif_poll_enable(x)
static inline void _kc_ixgbe_netif_poll_enable(struct net_device *netdev)
{
...
#define netif_tx_disable(x) _kc_ixgbe_netif_tx_disable(x)
static inline void _kc_ixgbe_netif_tx_disable(struct net_device *dev)
{
...
#define netif_msg_init _kc_ixgbe_netif_msg_init
static inline u32 _kc_ixgbe_netif_msg_init(int debug_value, int default_msg_enable_bits)
{
	/* use default */
...
#ifndef rcu_head
struct __kc_ixgbe_callback_head {
	struct __kc_ixgbe_callback_head *next;
	void (*func)(struct callback_head *head);
};
#define rcu_head __kc_ixgbe_callback_head
#endif
...
#define skb_fill_page_desc _kc_ixgbe_skb_fill_page_desc
void _kc_ixgbe_skb_fill_page_desc(struct sk_buff *skb, int i, struct page *page, int off, int size);
#endif
...
#undef find_next_bit
#define find_next_bit _kc_ixgbe_find_next_bit
unsigned long _kc_ixgbe_find_next_bit(const unsigned long *addr, unsigned long size,
				unsigned long offset);
#define find_first_bit(addr, size) find_next_bit((addr), (size), 0)

#ifndef netdev_name
static inline const char *_kc_ixgbe_netdev_name(const struct net_device *dev)
{
	if (strchr(dev->name, '%'))
		return "(unregistered net_device)";
	return dev->name;
}
#define netdev_name(netdev)	_kc_ixgbe_netdev_name(netdev)
#endif /* netdev_name */

#ifndef strlcpy
#define strlcpy _kc_ixgbe_strlcpy
size_t _kc_ixgbe_strlcpy(char *dest, const char *src, size_t size);
#endif /* strlcpy */
...
uint32_t _kc_ixgbe__div64_32(uint64_t *dividend, uint32_t divisor);
# define do_div(n,base) ({				\
	uint32_t __base = (base);			\
	uint32_t __rem;					\
	if (likely(((n) >> 32) == 0)) {			\
		...
		__rem = _kc_ixgbe__div64_32(&(n), __base);	\
	__rem;						\
 })
#else /* BITS_PER_LONG == ?? */
...
#define pci_dma_mapping_error _kc_ixgbe_pci_dma_mapping_error
static inline int _kc_ixgbe_pci_dma_mapping_error(dma_addr_t dma_addr)
{
	return dma_addr == 0;
}
#endif
...
/*****************************************************************************/
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,4) )
int _kc_ixgbe_scnprintf(char * buf, size_t size, const char *fmt, ...);
#define scnprintf(buf, size, fmt, args...) _kc_ixgbe_scnprintf(buf, size, fmt, ##args)
#endif /* < 2.6.4 */
...
#undef bitmap_zero
#define bitmap_zero _kc_ixgbe_bitmap_zero
static inline void _kc_ixgbe_bitmap_zero(unsigned long *dst, int nbits)
{
...
#undef if_mii
#define if_mii _kc_ixgbe_if_mii
static inline struct mii_ioctl_data *_kc_ixgbe_if_mii(struct ifreq *rq)
{
...
#ifndef kcalloc
#define kcalloc(n, size, flags) _kc_ixgbe_kzalloc(((n) * (size)), flags)
void *_kc_ixgbe_kzalloc(size_t size, int flags);
#endif
#define MSEC_PER_SEC    1000L
static inline unsigned int _kc_ixgbe_jiffies_to_msecs(const unsigned long j)
{
...
static inline unsigned long _kc_ixgbe_msecs_to_jiffies(const unsigned int m)
{
	if (m > _kc_ixgbe_jiffies_to_msecs(MAX_JIFFY_OFFSET))
...
#define msleep_interruptible _kc_ixgbe_msleep_interruptible
static inline unsigned long _kc_ixgbe_msleep_interruptible(unsigned int msecs)
{
	unsigned long timeout = _kc_ixgbe_msecs_to_jiffies(msecs) + 1;

	while (timeout && !signal_pending(current)) {
		__set_current_state(TASK_INTERRUPTIBLE);
		timeout = schedule_timeout(timeout);
	}
	return _kc_ixgbe_jiffies_to_msecs(timeout);
}
...
#define skb_header_pointer _kc_ixgbe_skb_header_pointer
static inline void *_kc_ixgbe_skb_header_pointer(const struct sk_buff *skb,
...
extern DECLARE_BITMAP(_kc_ixgbeompat_node_online_map, MAX_NUMNODES);
#undef node_online_map
#define node_online_map _kc_ixgbeompat_node_online_map
...
#define usecs_to_jiffies _kc_ixgbe_usecs_to_jiffies
static inline unsigned int _kc_ixgbe_jiffies_to_usecs(const unsigned long j)
{
...
static inline unsigned long _kc_ixgbe_usecs_to_jiffies(const unsigned int m)
{
	if (m > _kc_ixgbe_jiffies_to_usecs(MAX_JIFFY_OFFSET))
		return MAX_JIFFY_OFFSET;
#if HZ <= KC_USEC_PER_SEC && !(KC_USEC_PER_SEC % HZ)
...
#define is_zero_ether_addr _kc_ixgbe_is_zero_ether_addr
static inline int _kc_ixgbe_is_zero_ether_addr(const u8 *addr)
{
	return !(addr[0] | addr[1] | addr[2] | addr[3] | addr[4] | addr[5]);
}
#endif /* is_zero_ether_addr */
#ifndef is_multicast_ether_addr
#define is_multicast_ether_addr _kc_ixgbe_is_multicast_ether_addr
static inline int _kc_ixgbe_is_multicast_ether_addr(const u8 *addr)
{
	return addr[0] & 0x01;
}
...
#ifndef kstrdup
#define kstrdup _kc_ixgbe_kstrdup
char *_kc_ixgbe_kstrdup(const char *s, unsigned int gfp);
#endif
#endif /* < 2.6.13 */
...
#ifndef kzalloc
#define kzalloc _kc_ixgbe_kzalloc
void *_kc_ixgbe_kzalloc(size_t size, int flags);
#endif
...
#endif
static inline unsigned _kc_ixgbe_compare_ether_addr(const u8 *addr1, const u8 *addr2)
{
...
#define compare_ether_addr(addr1, addr2) _kc_ixgbe_compare_ether_addr(addr1, addr2)
#endif /* < 2.6.15 */
...
#ifndef skb_is_gso
#ifdef NETIF_F_TSO
#define skb_is_gso _kc_ixgbe_skb_is_gso
static inline int _kc_ixgbe_skb_is_gso(const struct sk_buff *skb)
{
...
#undef skb_pad
#endif
#define skb_pad(x,y) _kc_ixgbe_skb_pad(x, y)
int _kc_ixgbe_skb_pad(struct sk_buff *skb, int pad);
#ifdef skb_padto
#undef skb_padto
#endif
#define skb_padto(x,y) _kc_ixgbe_skb_padto(x, y)
static inline int _kc_ixgbe_skb_padto(struct sk_buff *skb, unsigned int len)
{
	unsigned int size = skb->len;
	if(likely(size >= len))
		return 0;
	return _kc_ixgbe_skb_pad(skb, len - size);
}
...
static inline irqreturn_t _kc_ixgbe_request_irq(unsigned int irq, new_handler_t handler, unsigned long flags, const char *devname, void *dev_id)
#else /* 2.4.x */
typedef void (*irq_handler_t)(int, void*, struct pt_regs *);
typedef void (*new_handler_t)(int, void*);
static inline int _kc_ixgbe_request_irq(unsigned int irq, new_handler_t handler, unsigned long flags, const char *devname, void *dev_id)
#endif /* >= 2.5.x */
{
...
#undef request_irq
#define request_irq(irq, handler, flags, devname, dev_id) _kc_ixgbe_request_irq((irq), (handler), (flags), (devname), (dev_id))

#define irq_handler_t new_handler_t

#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,11) )
#ifndef skb_checksum_help
static inline int __kc_ixgbe_skb_checksum_help(struct sk_buff *skb)
{
	return skb_checksum_help(skb, 0);
}
#define skb_checksum_help(skb) __kc_ixgbe_skb_checksum_help((skb))
#endif
#endif /* < 2.6.19 && >= 2.6.11 */
...
int _kc_ixgbe_pci_save_state(struct pci_dev *);
#define pci_save_state(pdev) _kc_ixgbe_pci_save_state(pdev)
#undef pci_restore_state
void _kc_ixgbe_pci_restore_state(struct pci_dev *);
#define pci_restore_state(pdev) _kc_ixgbe_pci_restore_state(pdev)
#endif /* !(RHEL_RELEASE_CODE >= RHEL 5.4) */

#ifdef HAVE_PCI_ERS
#undef free_netdev
void _kc_ixgbe_free_netdev(struct net_device *);
#define free_netdev(netdev) _kc_ixgbe_free_netdev(netdev)
#endif
...
void *_kc_ixgbe_kmemdup(const void *src, size_t len, unsigned gfp);
#define kmemdup(src, len, gfp) _kc_ixgbe_kmemdup(src, len, gfp)
#ifndef bool
#define bool _Bool
#define true 1
#define false 0
...
struct pci_dev *_kc_ixgbe_netdev_to_pdev(struct net_device *netdev);
#define netdev_to_dev(netdev)	\
	pci_dev_to_dev(_kc_ixgbe_netdev_to_pdev(netdev))
...
void _kc_ixgbe_print_hex_dump(const char *level, const char *prefix_str,
			int prefix_type, int rowsize, int groupsize,
			const void *buf, size_t len, bool ascii);
#define print_hex_dump(lvl, s, t, r, g, b, l, a) \
		_kc_ixgbe_print_hex_dump(lvl, s, t, r, g, b, l, a)
...
static inline int __kc_ixgbe_skb_cow_head(struct sk_buff *skb, unsigned int headroom)
{
	int delta = 0;
...
#define skb_cow_head(s, h) __kc_ixgbe_skb_cow_head((s), (h))
#endif /* < 2.6.23 */
...
#ifdef NAPI
int __kc_ixgbe_adapter_clean(struct net_device *, int *);
...
static inline void __kc_ixgbe_mq_netif_napi_add(struct net_device *dev, struct napi_struct *napi,
					  int (*poll)(struct napi_struct *, int), int weight)
{
	struct net_device *poll_dev = napi_to_poll_dev(napi);
	poll_dev->poll = __kc_ixgbe_adapter_clean;
	poll_dev->priv = napi;
...
#define netif_napi_add __kc_ixgbe_mq_netif_napi_add

static inline void __kc_ixgbe_mq_netif_napi_del(struct napi_struct *napi)
{
	struct net_device *poll_dev = napi_to_poll_dev(napi);
...
#define netif_napi_del __kc_ixgbe_mq_netif_napi_del

static inline bool __kc_ixgbe_mq_napi_schedule_prep(struct napi_struct *napi)
{
	return netif_running(napi->dev) &&
		netif_rx_schedule_prep(napi_to_poll_dev(napi));
}
#define napi_schedule_prep __kc_ixgbe_mq_napi_schedule_prep

static inline void __kc_ixgbe_mq_napi_schedule(struct napi_struct *napi)
{
	if (napi_schedule_prep(napi))
		__netif_rx_schedule(napi_to_poll_dev(napi));
}
#define napi_schedule __kc_ixgbe_mq_napi_schedule
...
static inline void _kc_ixgbe_napi_complete(struct napi_struct *napi)
{
#ifdef NETIF_F_GRO
	napi_gro_flush(napi);
#endif
...
#define napi_complete _kc_ixgbe_napi_complete
#else /* NAPI */

/* The following definitions are only used if we don't support NAPI at all. */

static inline __kc_ixgbe_netif_napi_add(struct net_device *dev, struct napi_struct *napi,
				  int (*poll)(struct napi_struct *, int), int weight)
{
	dev->poll = poll;
	dev->weight = weight;
...
#ifdef NETIF_F_TSO6
#define skb_is_gso_v6 _kc_ixgbe_skb_is_gso_v6
static inline int _kc_ixgbe_skb_is_gso_v6(const struct sk_buff *skb)
{
...
#define strict_strtol(s, b, r) _kc_ixgbe_strict_strtol(s, b, r)
static inline int _kc_ixgbe_strict_strtol(const char *buf, unsigned int base, long *res)
{
...
void _kc_ixgbe_pci_disable_link_state(struct pci_dev *dev, int state);
#define pci_disable_link_state(p, s) _kc_ixgbe_pci_disable_link_state(p, s)
#else /* < 2.6.26 */
...
/*****************************************************************************/
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,27) )
static inline void _kc_ixgbe_ethtool_cmd_speed_set(struct ethtool_cmd *ep,
					     __u32 speed)
{
	ep->speed = (__u16)speed;
	/* ep->speed_hi = (__u16)(speed >> 16); */
}
#define ethtool_cmd_speed_set _kc_ixgbe_ethtool_cmd_speed_set

static inline __u32 _kc_ixgbe_ethtool_cmd_speed(struct ethtool_cmd *ep)
{
	/* no speed_hi before 2.6.27, and probably no need for it yet */
	return (__u32)ep->speed;
}
#define ethtool_cmd_speed _kc_ixgbe_ethtool_cmd_speed
...
#ifdef HAVE_TX_MQ
void _kc_ixgbe_netif_tx_stop_all_queues(struct net_device *);
void _kc_ixgbe_netif_tx_wake_all_queues(struct net_device *);
void _kc_ixgbe_netif_tx_start_all_queues(struct net_device *);
#define netif_tx_stop_all_queues(a) _kc_ixgbe_netif_tx_stop_all_queues(a)
#define netif_tx_wake_all_queues(a) _kc_ixgbe_netif_tx_wake_all_queues(a)
#define netif_tx_start_all_queues(a) _kc_ixgbe_netif_tx_start_all_queues(a)
...
void __kc_ixgbe_warn_slowpath(const char *file, const int line,
		const char *fmt, ...) __attribute__((format(printf, 3, 4)));
#define __WARN_printf(arg...) __kc_ixgbe_warn_slowpath(__FILE__, __LINE__, arg)
#endif /* __WARN_printf */
...
#define pci_wake_from_d3 _kc_ixgbe_pci_wake_from_d3
#define pci_prepare_to_sleep _kc_ixgbe_pci_prepare_to_sleep
int _kc_ixgbe_pci_wake_from_d3(struct pci_dev *dev, bool enable);
int _kc_ixgbe_pci_prepare_to_sleep(struct pci_dev *dev);
#define netdev_alloc_page(a) alloc_page(GFP_ATOMIC)
#ifndef __skb_queue_head_init
static inline void __kc_ixgbe_skb_queue_head_init(struct sk_buff_head *list)
{
	list->prev = list->next = (struct sk_buff *)list;
	list->qlen = 0;
}
#define __skb_queue_head_init(_q) __kc_ixgbe_skb_queue_head_init(_q)
#endif
...
void _kc_ixgbe_pci_clear_master(struct pci_dev *dev);
#define pci_clear_master(dev)	_kc_ixgbe_pci_clear_master(dev)
#endif
...
#define skb_tx_hash(n, s) ___kc_ixgbe_skb_tx_hash((n), (s), (n)->real_num_tx_queues)
#undef CONFIG_FCOE
#undef CONFIG_FCOE_MODULE
...
#endif /* pr_cont */
static inline void _kc_ixgbe_synchronize_irq(unsigned int a)
{
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,5,28) )
	synchronize_irq();
#else /* < 2.5.28 */
...
#define synchronize_irq(a) _kc_ixgbe_synchronize_irq(a)
...
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0) )
static inline int _kc_ixgbe_pm_runtime_get_sync()
{
	return 1;
}
#define pm_runtime_get_sync(dev)	_kc_ixgbe_pm_runtime_get_sync()
#else /* 2.6.0 => 2.6.32 */
static inline int _kc_ixgbe_pm_runtime_get_sync(struct device __always_unused *dev)
{
	return 1;
}
#ifndef pm_runtime_get_sync
#define pm_runtime_get_sync(dev)	_kc_ixgbe_pm_runtime_get_sync(dev)
#endif
#endif /* 2.6.0 => 2.6.32 */
...
#ifndef pci_num_vf
#define pci_num_vf(pdev) _kc_ixgbe_pci_num_vf(pdev)
int _kc_ixgbe_pci_num_vf(struct pci_dev *dev);
#endif
#endif /* RHEL_RELEASE_CODE */
...
do {								\
	struct pci_dev *pdev = _kc_ixgbe_netdev_to_pdev(netdev);	\
...
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0) )
static inline bool _kc_ixgbe_pm_runtime_suspended()
{
	return false;
}
#define pm_runtime_suspended(dev)	_kc_ixgbe_pm_runtime_suspended()
#else /* 2.6.0 => 2.6.34 */
static inline bool _kc_ixgbe_pm_runtime_suspended(struct device __always_unused *dev)
{
	return false;
}
#ifndef pm_runtime_suspended
#define pm_runtime_suspended(dev)	_kc_ixgbe_pm_runtime_suspended(dev)
#endif
#endif /* 2.6.0 => 2.6.34 */

#ifndef pci_bus_speed
/* override pci_bus_speed introduced in 2.6.19 with an expanded enum type */
enum _kc_ixgbe_pci_bus_speed {
	_KC_PCIE_SPEED_2_5GT		= 0x14,
	_KC_PCIE_SPEED_5_0GT		= 0x15,
	_KC_PCIE_SPEED_8_0GT		= 0x16,
	_KC_PCI_SPEED_UNKNOWN		= 0xff,
};
#define pci_bus_speed		_kc_ixgbe_pci_bus_speed
...
ssize_t _kc_ixgbe_simple_write_to_buffer(void *to, size_t available, loff_t *ppos,
				   const void __user *from, size_t count);
#define simple_write_to_buffer _kc_ixgbe_simple_write_to_buffer
...
int _kc_ixgbe_netif_set_real_num_tx_queues(struct net_device *, unsigned int);
#else /* CONFIG_NETDEVICES_MULTI_QUEUE */
static inline int _kc_ixgbe_netif_set_real_num_tx_queues(struct net_device *dev,
						   unsigned int txq)
{
...
#else /* HAVE_TX_MQ */
static inline int _kc_ixgbe_netif_set_real_num_tx_queues(struct net_device __always_unused *dev,
...
#define netif_set_real_num_tx_queues(dev, txq) \
	_kc_ixgbe_netif_set_real_num_tx_queues(dev, txq)
...
int _kc_ixgbe_ethtool_op_set_flags(struct net_device *, u32, u32);
#define ethtool_op_set_flags _kc_ixgbe_ethtool_op_set_flags
u32 _kc_ixgbe_ethtool_op_get_flags(struct net_device *);
#define ethtool_op_get_flags _kc_ixgbe_ethtool_op_get_flags
...
static inline struct sk_buff *_kc_ixgbe_netdev_alloc_skb_ip_align(struct net_device *dev,
							    unsigned int length)
{
	struct sk_buff *skb;
...
#endif
#define netdev_alloc_skb_ip_align(n, l) _kc_ixgbe_netdev_alloc_skb_ip_align(n, l)
...
static inline int __kc_ixgbe_netif_set_real_num_rx_queues(struct net_device __always_unused *dev,
						    unsigned int __always_unused rxq)
{
	return 0;
}
#define netif_set_real_num_rx_queues(dev, rxq) \
	__kc_ixgbe_netif_set_real_num_rx_queues((dev), (rxq))
#endif
...
static inline void _kc_ixgbe_skb_checksum_none_assert(struct sk_buff *skb)
{
	WARN_ON(skb->ip_summed != CHECKSUM_NONE);
}
#define skb_checksum_none_assert(skb) _kc_ixgbe_skb_checksum_none_assert(skb)

static inline void *_kc_ixgbe_vzalloc_node(unsigned long size, int node)
{
...
#define vzalloc_node(_size, _node) _kc_ixgbe_vzalloc_node(_size, _node)

static inline void *_kc_ixgbe_vzalloc(unsigned long size)
{
...
#define vzalloc(_size) _kc_ixgbe_vzalloc(_size)
...
#else /* 2.6.22 -> 2.6.37 */
static inline int _kc_ixgbe_skb_checksum_start_offset(const struct sk_buff *skb)
{
        return skb->csum_start - skb_headroom(skb);
}
#define skb_checksum_start_offset(skb) _kc_ixgbe_skb_checksum_start_offset(skb)
#endif /* 2.6.22 -> 2.6.37 */
...
#if (!(RHEL_RELEASE_CODE && RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(6,0)))
u16 ___kc_ixgbe_skb_tx_hash(struct net_device *, const struct sk_buff *, u16);
#define __skb_tx_hash(n, s, q) ___kc_ixgbe_skb_tx_hash((n), (s), (q))
u8 _kc_ixgbe_netdev_get_num_tc(struct net_device *dev);
#define netdev_get_num_tc(dev) _kc_ixgbe_netdev_get_num_tc(dev)
int _kc_ixgbe_netdev_set_num_tc(struct net_device *dev, u8 num_tc);
#define netdev_set_num_tc(dev, tc) _kc_ixgbe_netdev_set_num_tc((dev), (tc))
#define netdev_reset_tc(dev) _kc_ixgbe_netdev_set_num_tc((dev), 0)
#define netdev_set_tc_queue(dev, tc, cnt, off) do {} while (0)
u8 _kc_ixgbe_netdev_get_prio_tc_map(struct net_device *dev, u8 up);
#define netdev_get_prio_tc_map(dev, up) _kc_ixgbe_netdev_get_prio_tc_map(dev, up)
...
#ifndef udp_csum
#define udp_csum __kc_ixgbe_udp_csum
static inline __wsum __kc_ixgbe_udp_csum(struct sk_buff *skb)
{
...
union _kc_ixgbe_ethtool_flow_union {
	struct ethtool_tcpip4_spec		tcp_ip4_spec;
	struct ethtool_usrip4_spec		usr_ip4_spec;
	__u8					hdata[60];
};
struct _kc_ixgbe_ethtool_flow_ext {
	__be16	vlan_etype;
	__be16	vlan_tci;
	__be32	data[2];
};
struct _kc_ixgbe_ethtool_rx_flow_spec {
	__u32		flow_type;
	union _kc_ixgbe_ethtool_flow_union h_u;
	struct _kc_ixgbe_ethtool_flow_ext h_ext;
	union _kc_ixgbe_ethtool_flow_union m_u;
	struct _kc_ixgbe_ethtool_flow_ext m_ext;
	__u64		ring_cookie;
	__u32		location;
};
#define ethtool_rx_flow_spec _kc_ixgbe_ethtool_rx_flow_spec
#endif /* FLOW_EXT */
#endif
...
#define kstrtol_from_user(s, c, b, r) _kc_ixgbe_kstrtol_from_user(s, c, b, r)
static inline int _kc_ixgbe_kstrtol_from_user(const char __user *s, size_t count,
...
#define dma_zalloc_coherent(d, s, h, f) _kc_ixgbe_dma_zalloc_coherent(d, s, h, f)
static inline void *_kc_ixgbe_dma_zalloc_coherent(struct device *dev, size_t size,
...
#define skb_frag_size(frag)	_kc_ixgbe_skb_frag_size(frag)
static inline unsigned int _kc_ixgbe_skb_frag_size(const skb_frag_t *frag)
{
	return frag->size;
}
#endif /* skb_frag_size */
...
#define skb_frag_size_sub(frag, delta)	_kc_ixgbe_skb_frag_size_sub(frag, delta)
static inline void _kc_ixgbe_skb_frag_size_sub(skb_frag_t *frag, int delta)
{
	frag->size -= delta;
}
...
#define skb_frag_page(frag)	_kc_ixgbe_skb_frag_page(frag)
static inline struct page *_kc_ixgbe_skb_frag_page(const skb_frag_t *frag)
{
	return frag->page;
}
#endif /* skb_frag_page */
...
#define skb_frag_address(frag)	_kc_ixgbe_skb_frag_address(frag)
static inline void *_kc_ixgbe_skb_frag_address(const skb_frag_t *frag)
{
...
#define skb_frag_dma_map(dev,frag,offset,size,dir) \
		_kc_ixgbe_skb_frag_dma_map(dev,frag,offset,size,dir)
static inline dma_addr_t _kc_ixgbe_skb_frag_dma_map(struct device *dev,
...
#define __skb_frag_unref(frag) __kc_ixgbe_skb_frag_unref(frag)
static inline void __kc_ixgbe_skb_frag_unref(skb_frag_t *frag)
{
...
/* NOTE: the order of parameters to _kc_ixgbe_alloc_workqueue() is different than
...
_kc_ixgbe_alloc_workqueue(__maybe_unused int flags, __maybe_unused int max_active,
		    const char *fmt, ...)
{
...
#define alloc_workqueue(fmt, flags, max_active, args...) \
	_kc_ixgbe_alloc_workqueue(flags, max_active, fmt, ##args)
...
static inline int __kc_ixgbe_ipv6_skip_exthdr(const struct sk_buff *skb, int start,
					u8 *nexthdrp,
...
#define ipv6_skip_exthdr(a,b,c,d) __kc_ixgbe_ipv6_skip_exthdr((a), (b), (c), (d))
...
int _kc_ixgbe_simple_open(struct inode *inode, struct file *file);
#define simple_open _kc_ixgbe_simple_open
#endif /* !(SLE_VERSION_CODE && SLE_VERSION_CODE >= SLE_VERSION(11,3,0)) */

#ifndef skb_add_rx_frag
#define skb_add_rx_frag _kc_ixgbe_skb_add_rx_frag
void _kc_ixgbe_skb_add_rx_frag(struct sk_buff * skb, int i, struct page *page,
...
static inline bool __kc_ixgbe_ether_addr_equal(const u8 *addr1, const u8 *addr2)
{
	return !compare_ether_addr(addr1, addr2);
}
#define ether_addr_equal(_addr1, _addr2) __kc_ixgbe_ether_addr_equal((_addr1),(_addr2))
#endif
...
#define eth_broadcast_addr _kc_ixgbe_eth_broadcast_addr
static inline void _kc_ixgbe_eth_broadcast_addr(u8 *addr)
{
...
#define eth_random_addr _kc_ixgbe_eth_random_addr
static inline void _kc_ixgbe_eth_random_addr(u8 *addr)
{
 ...
 */
static inline u32 __kc_ixgbe_mmd_eee_cap_to_ethtool_sup_t(u16 eee_cap)
{
	u32 supported = 0;
...
#define mmd_eee_cap_to_ethtool_sup_t(eee_cap) \
	__kc_ixgbe_mmd_eee_cap_to_ethtool_sup_t(eee_cap)
 ...
 * settings.
 */
static inline u32 __kc_ixgbe_mmd_eee_adv_to_ethtool_adv_t(u16 eee_adv)
{
	u32 adv = 0;
...
#define mmd_eee_adv_to_ethtool_adv_t(eee_adv) \
	__kc_ixgbe_mmd_eee_adv_to_ethtool_adv_t(eee_adv)
 ...
 */
static inline u16 __kc_ixgbe_ethtool_adv_to_mmd_eee_adv_t(u32 adv)
{
	u16 reg = 0;
...
#define ethtool_adv_to_mmd_eee_adv_t(adv) __kc_ixgbe_ethtool_adv_to_mmd_eee_adv_t(adv)
...
int __kc_ixgbe_pcie_capability_read_word(struct pci_dev *dev, int pos, u16 *val);
#define pcie_capability_read_word(d,p,v) __kc_ixgbe_pcie_capability_read_word(d,p,v)
#endif /* pcie_capability_read_word */

#ifndef pcie_capability_read_dword
int __kc_ixgbe_pcie_capability_read_dword(struct pci_dev *dev, int pos, u32 *val);
#define pcie_capability_read_dword(d,p,v) __kc_ixgbe_pcie_capability_read_dword(d,p,v)
#endif

#ifndef pcie_capability_write_word
int __kc_ixgbe_pcie_capability_write_word(struct pci_dev *dev, int pos, u16 val);
#define pcie_capability_write_word(d,p,v) __kc_ixgbe_pcie_capability_write_word(d,p,v)
#endif /* pcie_capability_write_word */

#ifndef pcie_capability_clear_and_set_word
int __kc_ixgbe_pcie_capability_clear_and_set_word(struct pci_dev *dev, int pos,
					    u16 clear, u16 set);
#define pcie_capability_clear_and_set_word(d,p,c,s) \
	__kc_ixgbe_pcie_capability_clear_and_set_word(d,p,c,s)
#endif /* pcie_capability_clear_and_set_word */

#ifndef pcie_capability_clear_word
int __kc_ixgbe_pcie_capability_clear_word(struct pci_dev *dev, int pos,
					     u16 clear);
#define pcie_capability_clear_word(d, p, c) \
	__kc_ixgbe_pcie_capability_clear_word(d, p, c)
...
static inline int __kc_ixgbe_pci_sriov_set_totalvfs(struct pci_dev __always_unused *dev, u16 __always_unused numvfs)
{
	return 0;
}
#define pci_sriov_set_totalvfs(a, b) __kc_ixgbe_pci_sriov_set_totalvfs((a), (b))
#endif
...
static inline bool __kc_ixgbe_is_link_local_ether_addr(const u8 *addr)
{
...
#define is_link_local_ether_addr(addr) __kc_ixgbe_is_link_local_ether_addr(addr)
...
#ifdef CONFIG_XPS
int __kc_ixgbe_netif_set_xps_queue(struct net_device *, const struct cpumask *, u16);
#define netif_set_xps_queue(_dev, _mask, _idx) __kc_ixgbe_netif_set_xps_queue((_dev), (_mask), (_idx))
#else /* CONFIG_XPS */
#define netif_set_xps_queue(_dev, _mask, _idx) do {} while (0)
#endif /* CONFIG_XPS */

#ifdef HAVE_NETDEV_SELECT_QUEUE
#define _kc_ixgbe_hashrnd 0xd631614b /* not so random hash salt */
u16 __kc_ixgbe_netdev_pick_tx(struct net_device *dev, struct sk_buff *skb);
#define __netdev_pick_tx __kc_ixgbe_netdev_pick_tx
...
#endif
#ifdef CONFIG_PCI_IOV
int __kc_ixgbe_pci_vfs_assigned(struct pci_dev *dev);
#else
static inline int __kc_ixgbe_pci_vfs_assigned(struct pci_dev __always_unused *dev)
{
	return 0;
}
#endif
#define pci_vfs_assigned(dev) __kc_ixgbe_pci_vfs_assigned(dev)
...
static inline struct sk_buff *__kc_ixgbe__vlan_hwaccel_put_tag(struct sk_buff *skb,
							 u16 vlan_tci)
{
...
#define __vlan_hwaccel_put_tag(skb, vlan_proto, vlan_tci) \
	__kc_ixgbe__vlan_hwaccel_put_tag(skb, vlan_tci)
#endif

#ifdef HAVE_FDB_OPS
#if defined(HAVE_NDO_FDB_ADD_NLATTR)
int __kc_ixgbe_ndo_dflt_fdb_add(struct ndmsg *ndm, struct nlattr *tb[],
			  struct net_device *dev,
			  const unsigned char *addr, u16 flags);
#elif defined(USE_CONST_DEV_UC_CHAR)
int __kc_ixgbe_ndo_dflt_fdb_add(struct ndmsg *ndm, struct net_device *dev,
			  const unsigned char *addr, u16 flags);
#else
int __kc_ixgbe_ndo_dflt_fdb_add(struct ndmsg *ndm, struct net_device *dev,
...
int __kc_ixgbe_ndo_dflt_fdb_del(struct ndmsg *ndm, struct nlattr *tb[],
			  struct net_device *dev,
			  const unsigned char *addr);
#elif defined(USE_CONST_DEV_UC_CHAR)
int __kc_ixgbe_ndo_dflt_fdb_del(struct ndmsg *ndm, struct net_device *dev,
			  const unsigned char *addr);
#else
int __kc_ixgbe_ndo_dflt_fdb_del(struct ndmsg *ndm, struct net_device *dev,
			  unsigned char *addr);
#endif /* HAVE_FDB_DEL_NLATTR */
#define ndo_dflt_fdb_add __kc_ixgbe_ndo_dflt_fdb_add
#define ndo_dflt_fdb_del __kc_ixgbe_ndo_dflt_fdb_del
#endif /* HAVE_FDB_OPS */
...
int __kc_ixgbe_pcie_get_minimum_link(struct pci_dev *dev, enum pci_bus_speed *speed,
			       enum pcie_link_width *width);
#ifndef pcie_get_minimum_link
#define pcie_get_minimum_link(_p, _s, _w) __kc_ixgbe_pcie_get_minimum_link(_p, _s, _w)
#endif
#else /* >= 3.12.0 */
...
#define dma_set_mask_and_coherent(_p, _m) __kc_ixgbe_dma_set_mask_and_coherent(_p, _m)
int __kc_ixgbe_dma_set_mask_and_coherent(struct device *dev, u64 mask);
...
#if ( LINUX_VERSION_CODE > KERNEL_VERSION(2,6,20) )
#define devm_kc_ixgbealloc(dev, cnt, size, flags) \
...
#define pkt_hash_types         _kc_ixgbe_pkt_hash_types

#define skb_set_hash __kc_ixgbe_skb_set_hash
static inline void __kc_ixgbe_skb_set_hash(struct sk_buff __maybe_unused *skb,
...
int __kc_ixgbe_pci_enable_msix_range(struct pci_dev *dev, struct msix_entry *entries,
			       int minvec, int maxvec);
#define pci_enable_msix_range __kc_ixgbe_pci_enable_msix_range
#endif

#ifndef ether_addr_copy
#define ether_addr_copy __kc_ixgbe_ether_addr_copy
static inline void __kc_ixgbe_ether_addr_copy(u8 *dst, const u8 *src)
{
...
int __kc_ixgbe_ipv6_find_hdr(const struct sk_buff *skb, unsigned int *offset,
		       int target, unsigned short *fragoff, int *flags);
#define ipv6_find_hdr(a, b, c, d, e) __kc_ixgbe_ipv6_find_hdr((a), (b), (c), (d), (e))
...
char *_kc_ixgbe_devm_kstrdup(struct device *dev, const char *s, gfp_t gfp);
#define devm_kstrdup(dev, s, gfp) _kc_ixgbe_devm_kstrdup(dev, s, gfp)
...
int __kc_ixgbe_hw_addr_sync_dev(struct netdev_hw_addr_list *list,
		struct net_device *dev,
		int (*sync)(struct net_device *, const unsigned char *),
		int (*unsync)(struct net_device *, const unsigned char *));
void __kc_ixgbe_hw_addr_unsync_dev(struct netdev_hw_addr_list *list,
		struct net_device *dev,
		int (*unsync)(struct net_device *, const unsigned char *));
#endif
#ifndef NETDEV_HW_ADDR_T_MULTICAST
int __kc_ixgbe_dev_addr_sync_dev(struct dev_addr_list **list, int *count,
		struct net_device *dev,
		int (*sync)(struct net_device *, const unsigned char *),
		int (*unsync)(struct net_device *, const unsigned char *));
void __kc_ixgbe_dev_addr_unsync_dev(struct dev_addr_list **list, int *count,
		struct net_device *dev,
		int (*unsync)(struct net_device *, const unsigned char *));
#endif
#endif /* HAVE_SET_RX_MODE */

static inline int __kc_ixgbe_dev_uc_sync(struct net_device __maybe_unused *dev,
...
{
#ifdef NETDEV_HW_ADDR_T_UNICAST
	return __kc_ixgbe_hw_addr_sync_dev(&dev->uc, dev, sync, unsync);
#elif defined(HAVE_SET_RX_MODE)
	return __kc_ixgbe_dev_addr_sync_dev(&dev->uc_list, &dev->uc_count,
				      dev, sync, unsync);
#else
	return 0;
#endif
}
#define __dev_uc_sync __kc_ixgbe_dev_uc_sync

static inline void __kc_ixgbe_dev_uc_unsync(struct net_device __maybe_unused *dev,
				      int __maybe_unused (*unsync)(struct net_device *, const unsigned char *))
{
#ifdef HAVE_SET_RX_MODE
#ifdef NETDEV_HW_ADDR_T_UNICAST
	__kc_ixgbe_hw_addr_unsync_dev(&dev->uc, dev, unsync);
#else /* NETDEV_HW_ADDR_T_MULTICAST */
	__kc_ixgbe_dev_addr_unsync_dev(&dev->uc_list, &dev->uc_count, dev, unsync);
#endif /* NETDEV_HW_ADDR_T_UNICAST */
#endif /* HAVE_SET_RX_MODE */
}
#define __dev_uc_unsync __kc_ixgbe_dev_uc_unsync

static inline int __kc_ixgbe_dev_mc_sync(struct net_device __maybe_unused *dev,
				   int __maybe_unused (*sync)(struct net_device *, const unsigned char *),
				   int __maybe_unused (*unsync)(struct net_device *, const unsigned char *))
{
#ifdef NETDEV_HW_ADDR_T_MULTICAST
	return __kc_ixgbe_hw_addr_sync_dev(&dev->mc, dev, sync, unsync);
#elif defined(HAVE_SET_RX_MODE)
	return __kc_ixgbe_dev_addr_sync_dev(&dev->mc_list, &dev->mc_count,
				      dev, sync, unsync);
#else
	return 0;
#endif
...
#define __dev_mc_sync __kc_ixgbe_dev_mc_sync

static inline void __kc_ixgbe_dev_mc_unsync(struct net_device __maybe_unused *dev,
				      int __maybe_unused (*unsync)(struct net_device *, const unsigned char *))
{
#ifdef HAVE_SET_RX_MODE
#ifdef NETDEV_HW_ADDR_T_MULTICAST
	__kc_ixgbe_hw_addr_unsync_dev(&dev->mc, dev, unsync);
#else /* NETDEV_HW_ADDR_T_MULTICAST */
	__kc_ixgbe_dev_addr_unsync_dev(&dev->mc_list, &dev->mc_count, dev, unsync);
#endif /* NETDEV_HW_ADDR_T_MULTICAST */
#endif /* HAVE_SET_RX_MODE */
}
#define __dev_mc_unsync __kc_ixgbe_dev_mc_unsync
#endif /* __dev_uc_sync */
...
#endif
void *__kc_ixgbe_devm_kmemdup(struct device *dev, const void *src, size_t len,
			gfp_t gfp);
#define devm_kmemdup __kc_ixgbe_devm_kmemdup
...
struct sk_buff *__kc_ixgbe_skb_clone_sk(struct sk_buff *skb);
void __kc_ixgbe_skb_complete_tx_timestamp(struct sk_buff *skb,
				    struct skb_shared_hwtstamps *hwtstamps);
#define skb_clone_sk __kc_ixgbe_skb_clone_sk
#define skb_complete_tx_timestamp __kc_ixgbe_skb_complete_tx_timestamp
#endif
u32 __kc_ixgbe_eth_get_headlen(const struct net_device *dev, unsigned char *data,
			 unsigned int max_len);
#define eth_get_headlen __kc_ixgbe_eth_get_headlen
#ifndef ETH_P_XDSA
...
static inline void _kc_ixgbe_napi_complete_done(struct napi_struct *napi,
...
#define napi_complete_done _kc_ixgbe_napi_complete_done
#endif

int _kc_ixgbe_bitmap_print_to_pagebuf(bool list, char *buf,
				const unsigned long *maskp, int nmaskbits);
#define bitmap_print_to_pagebuf _kc_ixgbe_bitmap_print_to_pagebuf
...
#define NETDEV_RSS_KEY_LEN (13 * 4)
      ...
      ((RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(6,7) && RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7,0)) || \
       (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7,2)))))
#define netdev_rss_key_fill(buffer, len) __kc_ixgbe_netdev_rss_key_fill(buffer, len)
#endif /* RHEL_RELEASE_CODE */
void __kc_ixgbe_netdev_rss_key_fill(void *buffer, size_t len);
...
/**
 *     __kc_ixgbe_skb_put_padto - increase size and pad an skbuff up to a minimal size
 *     @skb: buffer to pad
 *     @len: minimal length
 *
 ...
 */
static inline int __kc_ixgbe_skb_put_padto(struct sk_buff *skb, unsigned int len)
{
	unsigned int size = skb->len;
...
#define skb_put_padto(skb, len) __kc_ixgbe_skb_put_padto(skb, len)

static inline int __kc_ixgbe_eth_skb_pad(struct sk_buff *skb)
{
	return __kc_ixgbe_skb_put_padto(skb, ETH_ZLEN);
}
#define eth_skb_pad(skb) __kc_ixgbe_eth_skb_pad(skb)
#endif /* eth_skb_pad && skb_put_padto */

#ifndef SKB_ALLOC_NAPI
/* RHEL 7.2 backported napi_alloc_skb and friends */
static inline struct sk_buff *__kc_ixgbe_napi_alloc_skb(struct napi_struct *napi, unsigned int length)
{
	return netdev_alloc_skb_ip_align(napi->dev, length);
}
#define napi_alloc_skb(napi,len) __kc_ixgbe_napi_alloc_skb(napi,len)
#define __napi_alloc_skb(napi,len,mask) __kc_ixgbe_napi_alloc_skb(napi,len)
...
#endif
static inline void __kc_ixgbe_timecounter_adjtime(struct timecounter *tc, s64 delta)
{
	tc->nsec += delta;
}
...
#define timecounter_adjtime __kc_ixgbe_timecounter_adjtime
#endif
#if ((RHEL_RELEASE_CODE && (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7,2))) || \
...
unsigned int _kc_ixgbe_cpumask_local_spread(unsigned int i, int node);
#define cpumask_local_spread _kc_ixgbe_cpumask_local_spread
#endif
#else /* >= 4,1,0 */
...
#if (!(RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7,3)))
#define eth_platform_get_mac_address _kc_ixgbe_eth_platform_get_mac_address
int _kc_ixgbe_eth_platform_get_mac_address(struct device *dev __maybe_unused,
...
/* SLES 12.3 and RHEL 7.5 backported this interface */
    ...
    (SLE_VERSION_CODE && (SLE_VERSION_CODE < SLE_VERSION(12,3,0))) || \
    (RHEL_RELEASE_CODE && (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7,5)))
static inline bool _kc_ixgbe_napi_complete_done2(struct napi_struct *napi,
					   int __always_unused work_done)
{
...
#endif
#define napi_complete_done _kc_ixgbe_napi_complete_done2
...
#define dma_map_page_attrs __kc_ixgbe_dma_map_page_attrs
static inline dma_addr_t __kc_ixgbe_dma_map_page_attrs(struct device *dev,
						 struct page *page,
						 size_t offset, size_t size,
						 enum dma_data_direction dir,
...
#define dma_unmap_page_attrs __kc_ixgbe_dma_unmap_page_attrs
static inline void __kc_ixgbe_dma_unmap_page_attrs(struct device *dev,
...
#endif
struct _kc_ixgbe_xdp_buff {
	void *data;
	void *data_end;
	void *data_hard_start;
};
#define xdp_buff _kc_ixgbe_xdp_buff
struct _kc_ixgbe_bpf_prog {
};
#define bpf_prog _kc_ixgbe_bpf_prog
#else /* > 4.14 */
...
/**
 * _kc_ixgbe_ethtool_ksettings_to_cmd - Convert ethtool_link_ksettings to ethtool_cmd
 ...
 * Hence, we keep it prefixed with _kc_ixgbe_ to make it clear this isn't actually
 * a real function in the kernel.
 */
static inline void
_kc_ixgbe_ethtool_ksettings_to_cmd(struct ethtool_link_ksettings *ks,
...
/*****************************************************************************/
     ...
     (SLE_VERSION_CODE && (SLE_VERSION_CODE <= SLE_VERSION(12,3,0))) || \
     (RHEL_RELEASE_CODE && (RHEL_RELEASE_CODE <= RHEL_RELEASE_VERSION(7,5))))
#define phy_speed_to_str _kc_ixgbe_phy_speed_to_str
const char *_kc_ixgbe_phy_speed_to_str(int speed);
...
void _kc_ixgbe_ethtool_intersect_link_masks(struct ethtool_link_ksettings *dst,
				      struct ethtool_link_ksettings *src);
#define ethtool_intersect_link_masks _kc_ixgbe_ethtool_intersect_link_masks
#else /* >= 4.15 */
#define HAVE_NDO_BPF
...
static inline unsigned long _kc_ixgbe_array_index_mask_nospec(unsigned long index,
							unsigned long size)
{
	/*
...
({									\
	...
	typeof(size) _s = (size);					\
	unsigned long _mask = _kc_ixgbe_array_index_mask_nospec(_i, _s);	\
									\
...
void _kc_ixgbe_pcie_print_link_status(struct pci_dev *dev);
#define pcie_print_link_status _kc_ixgbe_pcie_print_link_status
#else /* >= 4.17.0 */
...
#define macvlan_supports_dest_filter _kc_ixgbe_macvlan_supports_dest_filter
static inline bool _kc_ixgbe_macvlan_supports_dest_filter(struct net_device *dev)
{
...
#define macvlan_accel_priv _kc_ixgbe_macvlan_accel_priv
static inline void *_kc_ixgbe_macvlan_accel_priv(struct net_device *dev)
{
	struct macvlan_dev *macvlan = netdev_priv(dev);
...
#define macvlan_release_l2fw_offload _kc_ixgbe_macvlan_release_l2fw_offload
static inline int _kc_ixgbe_macvlan_release_l2fw_offload(struct net_device *dev)
{
	struct macvlan_dev *macvlan = netdev_priv(dev);
...
static inline char *__kc_ixgbe_xdp_umem_get_data(struct xdp_umem *umem, u64 addr)
{
	return umem->pages[addr >> PAGE_SHIFT].addr + (addr & (PAGE_SIZE - 1));
}

#define xdp_umem_get_data __kc_ixgbe_xdp_umem_get_data
#endif /* !xdp_umem_get_data */
#ifndef xdp_umem_get_dma
static inline dma_addr_t __kc_ixgbe_xdp_umem_get_dma(struct xdp_umem *umem, u64 addr)
{
	return umem->pages[addr >> PAGE_SHIFT].dma + (addr & (PAGE_SIZE - 1));
}

#define xdp_umem_get_dma __kc_ixgbe_xdp_umem_get_dma
...
static inline bool __kc_ixgbe_napi_if_scheduled_mark_missed(struct napi_struct *n)
{
...
#define napi_if_scheduled_mark_missed __kc_ixgbe_napi_if_scheduled_mark_missed
...
static inline u32
__kc_ixgbe_eth_get_headlen(const struct net_device __always_unused *dev, void *data,
		     unsigned int len)
{
	return eth_get_headlen(data, len);
}

#define eth_get_headlen(dev, data, len) __kc_ixgbe_eth_get_headlen(dev, data, len)
...
int _kc_ixgbe_flow_block_cb_setup_simple(struct flow_block_offload *f,
...
#define flow_block_cb_setup_simple(f, driver_list, cb, cb_ident, cb_priv, \
				   ingress_only) \
	_kc_ixgbe_flow_block_cb_setup_simple(f, driver_list, cb, cb_ident, cb_priv, \
				       ingress_only)
#endif /* HAVE_TC_CB_AND_SETUP_QDISC_MQPRIO */
#else /* >= 5.3.0 */
